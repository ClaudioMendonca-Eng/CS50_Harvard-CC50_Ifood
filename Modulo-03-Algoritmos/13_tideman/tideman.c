...
int main(int argc, string argv[])
{
    ...
    {
        // ranks[i] é a preferência do eleitor para o i-ésimo candidato
        int ranks[candidate_count];

        // Consulta para cada preferência
        for (int j = 0; j < candidate_count; j++)
        {
...
// Atualiza as preferências dadas uma nova votação
bool vote(int rank, string name, int ranks[])
{
    for (int i = 0; i < candidate_count; i++){
...
// Atualiza as preferências dada a classificação de um eleitor
void record_preferences(int ranks[])
{
    for(int i = 0; i < candidate_count; i++){
